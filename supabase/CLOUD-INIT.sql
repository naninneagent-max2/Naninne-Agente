...
-- =============================================================================
-- 3) SEED — 4 projects padrão
-- =============================================================================

-- Primeiro cria um user de seed (pra satisfazer FK)
DO $$
DECLARE
    seed_user_id uuid := gen_random_uuid();
BEGIN
    -- Insere user de seed (sem auth.users — relação lógica apenas)
    INSERT INTO public.users (id, email, display_name, preferences)
    VALUES (
        seed_user_id,
        'seed@naninne.local',
        'Seed User',
        '{"theme": "light", "default_project": "escrita"}'::jsonb
    )
    ON CONFLICT (id) DO NOTHING;

    -- 4 projects padrão
    INSERT INTO public.projects (id, slug, nome, cor, icone, descricao, user_id, is_default, ordem) VALUES
        (gen_random_uuid(), 'escrita', 'Escrita Criativa', '#3B82F6', 'BookOpen', 'Livros, capítulos, ensaios, ficção, poesia.', seed_user_id, true, 1),
        (gen_random_uuid(), 'audiovisual', 'Audiovisual', '#F97316', 'Film', 'Roteiros, cenas, prompts Midjourney, vídeos.', seed_user_id, true, 2),
        (gen_random_uuid(), 'mercado', 'Mercado', '#10B981', 'TrendingUp', 'Análise de planilhas, dados comerciais, apresentações.', seed_user_id, true, 3),
        (gen_random_uuid(), 'tech', 'Tech', '#64748B', 'Wrench', 'Desenvolvimento do próprio app, GitHub, Supabase.', seed_user_id, true, 4)
